...
import type { UserThemeData } from "@/lib/userTheme";

const PRESET_FONTS = ["Space Mono", "VT323", "Orbitron", "Cinzel"] as const;

interface Theme extends UserThemeData {
  id: string;
  name: string;
  isPublic: boolean;
  usageCount: number;
...
export default function ThemesPage() {
  ...
  const { customMeta, customVisible, view, syncFromProfile, disableCustomInHeader, enableCustomInHeader, resetToBase, onThemeDeleted } =
    useAppearance();
  const { locked: createLocked, run: runCreate } = useSubmitLock();
  const { locked: actionLocked, run: runAction } = useSubmitLock();
  ...
  const apply = (id: string) =>
    runAction(async () => {
      try {
        const theme = await api<Theme>(`/themes/${id}/apply`, { method: "POST" });
        syncFromProfile(theme, theme.id, theme.name);
  ...
  const removeTheme = (id: string, name: string) =>
    runDelete(async () => {
      if (!window.confirm(`Удалить тему «${name}»? Это действие нельзя отменить.`)) return;
      try {
        ...
        showToast(msg.includes("404") || msg.includes("Not Found") ? "Удаление недоступно — обновите сервер API" : msg, "error");
      }
    });
  ...
  const handleDisableCustom = () => {
    ...
    showToast("Кастомная тема скрыта из переключателя");
  };
  ...
  const hasActiveCustom = Boolean(customMeta);
  ...
  return (
    <div>
      ...
      >
        ...
        {user && (
          <Button onClick={() => setShowCreate(!showCreate)} disabled={createLocked}>
            + Создать тему
          </Button>
        )}
      </div>
      ...
      {user && (
        <Card style={{ marginBottom: "var(--space-4)" }}>
          <p style={{ fontSize: "14px", marginBottom: "10px", color: "var(--text-muted)" }}>
            Базовая тема сайта — <strong style={{ color: "var(--neon-cyan)" }}>Синтвейв</strong> (как при первом входе).
            {hasActiveCustom && (
              <>
                {" "}
                Сейчас применена кастомная: <strong style={{ color: "var(--neon-purple)" }}>{customMeta?.name}</strong>
                {view === "custom" && customVisible && (
                  <span style={{ color: "var(--neon-cyan)", marginLeft: "4px" }}>· включена</span>
                )}
                {!customVisible && (
                  <span style={{ marginLeft: "4px" }}>· скрыта из переключателя</span>
                )}
              </>
            )}
          </p>
          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
            <Button variant="ghost" onClick={handleResetToBase} disabled={actionLocked}>
              Вернуть базовую тему (Синтвейв)
            </Button>
            {hasActiveCustom && customVisible && view === "custom" && (
              <Button variant="ghost" onClick={handleDisableCustom} disabled={actionLocked}>
                Отключить кастомную тему
              </Button>
            )}
            {hasActiveCustom && !customVisible && (
              <Button
                onClick={() => {
                  enableCustomInHeader();
                  showToast("Тема снова в переключателе");
                }}
              >
                Показать в переключателе
              </Button>
            )}
          </div>
        </Card>
      ...
      {showCreate && (
        <Card style={{ marginBottom: "16px" }}>
          <Input
            placeholder="Название темы (мин. 2 символа)"
            value={form.name}
            onChange={(e) => setForm({ ...form, name: e.target.value })}
          />
          <div style={{ display: "flex", gap: "12px", marginTop: "10px", flexWrap: "wrap", alignItems: "center" }}>
            <label>
              Акцент: <input type="color" value={form.accentColor} onChange={(e) => setForm({ ...form, accentColor: e.target.value })} />
            </label>
            <label>
              Фон: <input type="color" value={form.backgroundColor} onChange={(e) => setForm({ ...form, backgroundColor: e.target.value })} />
            </label>
          </div>
          ...
          <Button onClick={create} disabled={createLocked} style={{ marginTop: "12px" }}>
            {createLocked ? "Создание…" : form.isPublic ? "Создать и опубликовать" : "Создать тему"}
          </Button>
        </Card>
      )}
      ...
      <div style={{ display: "grid", gap: "12px" }}>
        {themes.length === 0 && (
          <Card>
            <p style={{ color: "var(--text-muted)" }}>Тем пока нет</p>
          </Card>
        )}
        {themes.map((t) => (
          <Card key={t.id}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "12px" }}>
              <div>
                <h2 style={{ fontFamily: t.fontUrl ? t.fontFamily : t.fontFamily, color: t.accentColor }}>
                  {t.name}
                  {customMeta?.id === t.id && (
                    <span style={{ fontSize: "12px", color: "var(--neon-cyan)", marginLeft: "8px", fontWeight: 400 }}>· активна</span>
                  )}
                </h2>
                <div style={{ display: "flex", gap: "8px", alignItems: "center", marginTop: "4px", flexWrap: "wrap" }}>
                  <Avatar url={t.author.avatarUrl} nickname={t.author.nickname} size={24} />
                  <Link href={`/profile/${t.author.nickname}`} style={{ fontSize: "12px" }}>
                    {t.author.nickname}
                  </Link>
              ...
              <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                {user && tab === "mine" && (
                  <>
                    ...
                    <Button variant="danger" onClick={() => removeTheme(t.id, t.name)} disabled={deleteLocked}>
                      {deleteLocked ? "…" : "Удалить"}
                    </Button>
                  </>
                )}
                {user && (
                  <Button onClick={() => apply(t.id)} disabled={actionLocked}>
                    Применить
                  </Button>
                )}
              </div>
